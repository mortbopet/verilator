...
public:
    ...
    virtual void visit(AstNodeSpecResolve* nodep) override { assert(false && "unimplemented"); }
    virtual void visit(AstSpecResolveBool* nodep) override {
        // Await production of speculated variable
        puts("vlTOPp->__Vm_mt_" + cvtToStr(nodep->thisMTask()->id())
             + ".waitUntilUpstreamSpecDone(even_cycle);\n");
        ...
        // For any downstream mtask, bump its counter by 2 (speculative resolution implies that
        // both edges of the speculation was handled, given that commitment of speculative tasks
        // are mutually exclusive). Notice also, that we do this regardlessly of whether it is a
        // cross-thread dependency or not.
        for (V3GraphEdge* edgep = nodep->thisMTask()->outBeginp(); edgep;
             edgep = edgep->outNextp()) {
            const ExecMTask* nextp = dynamic_cast<ExecMTask*>(edgep->top());
            puts("vlTOPp->__Vm_mt_" + cvtToStr(nextp->id())
                 + ".signalUpstreamDone<2>(even_cycle);\n");
        }

        // For any downstream speculative mtasks (ie. tasks which speculated a variable
        // generated by the current task) who are still live, bump its speculate counter.
        // We signal regardlessly of whether the task is on the same thread or not (to make
        // dependency counter logic valid).
        for (uint32_t mtaskid : nodep->thisMTask()->downstreamSpeculativeMTasks()) {
...
class EmitCImp final : EmitCStmts {
    ...
    void emitMTaskBody(AstMTaskBody* nodep) {
        ...
        if (curExecMTaskp->speculative() == ExecMTask::Speculative::None) {
            ...
            // We signal regardlessly of whether the task is on the same thread or not (to make
            // dependency counter logic valid).
            for (uint32_t mtaskid : curExecMTaskp->downstreamSpeculativeMTasks()) {
                auto* mtaskp = v3Global.rootp()->execGraphp()->idToExecMTaskp(mtaskid);
                if (mtaskp == nullptr) { continue; }
...
void EmitCImp::emitMTaskVertexCtors(bool* firstp) {
    ...
    puts("__Vm_mt_final(" + cvtToStr(finalEdgesInCt)
         + ", 0" /* no speculative incoming edges*/ + ")");
