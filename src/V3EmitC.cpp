...
public:
    ...
    virtual void visit(AstSpecResolveBool* nodep) override {
        ...
        // counter by 2 (speculative resolution implies that both edges of the speculation was
        // handled, given that commitment of speculative tasks are mutually exclusive).
        for (V3GraphEdge* edgep = nodep->thisMTask()->outBeginp(); edgep;
             edgep = edgep->outNextp()) {
            const ExecMTask* nextp = dynamic_cast<ExecMTask*>(edgep->top());
            if (nextp->thread() != nodep->thisMTask()->thread()) {
    ...
    void emitMTaskBody(AstMTaskBody* nodep) {
        ...
        if (curExecMTaskp->speculative() == ExecMTask::Speculative::None) {
            ...
            for (ExecMTask* mtaskp : curExecMTaskp->downstreamSpeculativeMTasks()) {
                if (mtaskp->thread() != curExecMTaskp->thread()) {
    ...
    for (const V3GraphVertex* vxp = depGraphp->verticesBeginp(); vxp; vxp = vxp->verticesNextp()) {
        ...
        if (packedMTaskMayBlock(mtp) > 0) {
            ...
            puts("__Vm_mt_" + cvtToStr(mtp->id()) + "(" + cvtToStr(edgesInCt) + ", "
                 + (mtp->speculative() == ExecMTask::Speculative::None ? "0" : "1") + ")");
        }
